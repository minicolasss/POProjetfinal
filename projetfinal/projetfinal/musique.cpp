...
void play_music(const vector<pair<string, double>>& music, double speed_factor) { 
    for (const auto& note_duration : music) {
        string note = note_duration.first;
        double duration = note_duration.second * speed_factor;

        if (note == "0") {
            cout << " " << endl;
        }
        else if (note_to_frequency.find(note) != note_to_frequency.end()) {
            cout << "Note: " << note << " (Freq: " << note_to_frequency[note] << " Hz)" << endl;
        }
        else {
            cout << "Note inconnue: " << note << endl;
        }
